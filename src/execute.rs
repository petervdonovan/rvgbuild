...
macro_rules! unsafe_file_stem {
    ( $x:expr ) => { $x.file_stem().unwrap().to_str().unwrap() };
}
...
fn send_to_file(child: std::process::Child, ll: Vec<PathBuf>) {
    ...
    let output_file = last.parent().expect("Expected a file, not a root or empty path").join("src-gen").join(String::from(last.file_stem().expect("Expected file name").to_string_lossy()) + ".s");
    fs::create_dir_all(output_file.parent().expect("Output file must have a parent"));
    fs::write(&output_file, child.wait_with_output().expect("Failed to read stdout").stdout).expect("Failed to write to file");
}
